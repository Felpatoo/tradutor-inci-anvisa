...
1. Nunca diga que a tradução é oficial da ANVISA.
2. Nunca invente número CAS.
3. Preserve números, hífens, vírgulas e estruturas químicas.
4. Sugira português brasileiro somente quando tecnicamente plausível.
5. Se não tiver segurança, use null em traducao_sugerida.
6. Não altere o ingrediente original.
7. Confiança deve ser: alta, media ou baixa.
8. Se não houver tradução segura, confiança deve ser baixa.
9. Observação curta.
...
st.info(
    "✅ Tradução ANVISA = resultado da base consultada.\n\n"
...
with st.sidebar:
    ...
    if chave_via_secret:
        ...
        st.caption(
            "No localhost, você pode inserir "
            "a chave manualmente."
        )
...
if itens:
    ...
    if nao_encontrados:
        ...
        for ingrediente in nao_encontrados:
        ...
        if st.button(
            ...
            if not api_key:
                ...
                with st.spinner(
                    "Gemini analisando ingredientes..."
                ):
    ...
    if resultado_ia:
        ...
        st.warning(
            ...
            "com sugestões da IA e precisa de revisão."
        )
